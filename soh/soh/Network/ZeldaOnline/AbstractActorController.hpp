    ...
    class AbstractActorController {
      public:
       ...
       static void InstallCustomInit(Actor* actor, ActorFunc customInit) {
        ...
        virtual bool ShouldSendStandardProperties() const {
            return true;
        }
        static void DispatchDestroy(Actor* actor, PlayState* play);

        bool ShouldReinstateInit() const {
            return m_originalInit != nullptr;
        }
        ...
        static constexpr u8 CLAIM_REASON_PROXIMITY = 0;
        static constexpr u8 CLAIM_REASON_HIT = 1;

        void ClaimLeadership(u8 reason);
        ...
        bool IsMyOpenTextboxActor() const {
            return gPlayState != nullptr && gPlayState->msgCtx.msgMode != MSGMODE_NONE &&
                   gPlayState->msgCtx.talkActor == m_actor;
        }
        ...
        void UpdateAnimation(SkelAnime* skelAnime, bool lockFrame) {
            if (lockFrame) {
            ...
            *count = 0;
            ...
            while (now.BytesLeft() >= 1) {
                ...
                if (!changed) {
                    ...
                    changed = (plen != len) || (now.CompareRange(nowDataPos, prev, prevDataPos, len) != 0);
                    prev.Skip(plen);
                }
      ...
      protected:
        ...
        void BuildStandardProperties(ByteStream& out) {
            ...
            PackProperty(PROP_SHAPE_ROT, ByteStream() << PackedInt2(m_actor->shape.rot.x) << PackedInt2(m_actor->shape.rot.y) << PackedInt2(m_actor->shape.rot.z), out);
        }
        ...
        bool BuildStandardExtendedProperty(unsigned int which, ByteStream& out) {
            switch (which) {
                ...
                case PROP_SHAPE_YOFFSET:
                    ...
                    return true;
                case PROP_SCALE:
                    PackProperty(PROP_SCALE, ByteStream() << PackedFloat4(m_actor->scale.x) << PackedFloat4(m_actor->scale.y) << PackedFloat4(m_actor->scale.z), out);
                    return true;
                case PROP_VELOCITY:
                    PackProperty(PROP_VELOCITY, ByteStream() << PackedFloat4(m_actor->velocity.x) << PackedFloat4(m_actor->velocity.y) << PackedFloat4(m_actor->velocity.z) << PackedFloat4(m_actor->speedXZ), out);
                    return true;
                case PROP_FLAGS:
                ...
                case PROP_COLOR_FILTER:
                    PackProperty(PROP_COLOR_FILTER, ByteStream() << PackedUInt2(m_actor->colorFilterParams) << PackedUInt1(m_actor->colorFilterTimer), out);
                    return true;
                default:
                    return false;
            }
        ...
        ByteStream BuildAnimProperty(u8 animIndex, SkelAnime* skel) {
            return ByteStream() << PackedUInt1(animIndex) << PackedFloat4(skel->playSpeed)
                                << PackedInt2((s16)(skel->startFrame))
                                << PackedInt2((s16)(skel->endFrame)) << PackedFloat4(skel->animLength)
                                << PackedUInt1(skel->mode);
        }
        ...
        template <class T>
        static ByteStream& PackProperty(unsigned int index, const T& value, ByteStream& out) {
        ...
        virtual void WriteCustomState(ByteStream& packet) {
        }
        virtual void ReadCustomState(ByteStream& packet) {
        }

        void WriteProperties(ByteStream& out) {
            ByteStream full;

            if (ShouldSendStandardProperties())
        ...
        virtual void UpdatePuppet(PlayState* play) = 0;
